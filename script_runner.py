#!/usr/bin/env python3
"""
Script Runner UI – run scrape, preprocess, extract, and store from one dashboard.
Similar to https://github.com/tsegaimerhawi/tigrinya-agent
...
<head>
  ...
  <style>
    * {{ box-sizing: border-box; }}
    ...
    .scraper-options {{ display: flex; align-items: center; gap: 0.75rem; margin-bottom: 1.25rem; padding: 1rem; background: #18181b; border: 1px solid #3f3f46; border-radius: 8px; }}
...
<body>
  <div class="header-row">
    <div>
      ...
      <p class="sub">Scrape → Process → Ingest → Store. Run scripts and see live output.</p>
    </div>
  ...
  <script>
    ...
    async function saveConfig() {{
      ...
      try {{ await fetch('/api/config', {{ method: 'POST', headers: {{ 'Content-Type': 'application/json' }}, body: JSON.stringify(data) }}); configModal.classList.remove('open'); }} catch (e) {{ console.error(e); }}
    }}
  </script>
</body>
</html>"""
